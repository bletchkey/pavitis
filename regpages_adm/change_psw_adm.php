...
<?php
	...
	function getNewPassword($conn){
		...
		$result1 = mysqli_query($dbconn, "SELECT password FROM accountAdm WHERE id = '$id' ");
		...
		$psw = $row['password'];
			...
			if(strcmp($psw,$old_psw)== 0){
				...
				if($new_psw != $conf_new_psw){
					...
					header('Location: change_psw_adm.php?error=psw');

					exit();

				}else if (strlen($check_psw)<8){

					header('Location: change_psw_adm.php?error=shpsw');
				...
				$result2 = mysqli_query($dbconn, "UPDATE accountAdm SET password = '$conf_new_psw' WHERE id = '$id'");
				...
				header('Location: change_psw_adm.php?error=mispsw');
