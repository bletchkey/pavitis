...
<?php
	...
	";
		...
		$id_adm =  substr($url, strrpos($url, '=') + 1);

		$_SESSION['id_amministratore_modifica_dati'] = $id_adm;



		$get_dataAdm = mysqli_query($dbconn, "SELECT * FROM dataAdm WHERE id_accountAdm = '$id_adm' ");





		if (mysqli_num_rows($get_dataAdm) > 0)

		{

			$row = mysqli_fetch_assoc($get_dataAdm);
		...
		<form action='".editAdm($conn)."' method='POST'>
		...
		<select name='cittadinanza' class='combo-box' value='".$cittadinanza."'required>
		  ...
		  <option value='estera'>ESTERA</option>
			...
			<button class='main-button' type='submit' name='adm_editaccSubmit'>CONFERMA MODIFICHE</button>
