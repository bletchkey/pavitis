...
<?php
	...
	";
		...
		$id_docente =  substr($url, strrpos($url, '=') + 1);

		$_SESSION['id_docente_modifica_dati'] = $id_docente;



		$get_dataDoc = mysqli_query($dbconn, "SELECT * FROM dataDoc WHERE id_accountDoc = '$id_docente' ");





		if (mysqli_num_rows($get_dataDoc) > 0)

		{

			$row = mysqli_fetch_assoc($get_dataDoc);
		...
		<form action='".editDoc($conn)."' method='POST'>
		...
		<select name='cittadinanza' class='combo-box' value='".$cittadinanza."'required>
		  ...
		  <option value='estera'>ESTERA</option>
			...
			<button class='main-button' type='submit' name='doc_editaccSubmit'>CONFERMA MODIFICHE</button>
